...
class NaverFinanceCrawler:
    ...
    def get_finance_related_image(self, title, category):
        ...
        # 주식/증권 관련 키워드
        stock_keywords = ['주식', '코스피', '코스닥', '증권', '상장', '매수', '매도', '투자', '펀드', 'etf', '배당']
        # 은행/금융 관련 키워드
        bank_keywords = ['은행', '금리', '대출', '예금', '적금', '신용', '카드', '금융']
        # 부동산 관련 키워드
        real_estate_keywords = ['부동산', '아파트', '주택', '임대', '전세', '매매', '분양']
        # 암호화폐 관련 키워드
        crypto_keywords = ['비트코인', '이더리움', '암호화폐', '코인', '블록체인']
        # 경제 관련 키워드
        economy_keywords = ['경제', '물가', 'gdp', '인플레이션', '환율', '달러', '원화']
        # 기업/산업 관련 키워드
        business_keywords = ['기업', '회사', '산업', '제조', '서비스', '매출', '영업이익']
        
        # 키워드 매칭을 통한 이미지 선택
        if any(keyword in title_lower for keyword in stock_keywords) or category in ['주식', '증권']:
            return 'https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=400&h=300&fit=crop&auto=format'  # 주식 차트
        elif any(keyword in title_lower for keyword in bank_keywords) or category == '금융':
            return 'https://images.unsplash.com/photo-1554224155-6726b3ff858f?w=400&h=300&fit=crop&auto=format'  # 은행/금융
        elif any(keyword in title_lower for keyword in real_estate_keywords) or category == '부동산':
            return 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=400&h=300&fit=crop&auto=format'  # 부동산
        elif any(keyword in title_lower for keyword in crypto_keywords):
            return 'https://images.unsplash.com/photo-1639762681485-074b7f938ba0?w=400&h=300&fit=crop&auto=format'  # 암호화폐
        elif any(keyword in title_lower for keyword in economy_keywords) or category == '경제':
            return 'https://images.unsplash.com/photo-1559526324-4b87b5e36e44?w=400&h=300&fit=crop&auto=format'  # 경제/비즈니스
        elif any(keyword in title_lower for keyword in business_keywords) or category in ['산업', '벤처']:
            return 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&h=300&fit=crop&auto=format'  # 비즈니스/기업
        else:
            # 기본 금융 이미지
            return 'https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?w=400&h=300&fit=crop&auto=format'  # 일반 금융
