    ...
    <div class="container">
      <div class="content-wrapper" v-if="loaded">
        ...
        <div class="section" v-if="isOwnProfile">
          ...
          >
            <div
              v-for="stock in profile.interested_stocks"
              :key="stock.code"
              class="stock-card"
            >
              <div class="stock-info">
              ...
              <div class="stock-actions">
                <button
                  @click="removeStock('interested', stock.code)"
                  class="btn-remove"
                >
                  <i class="fas fa-times"></i>
                </button>
...
const loadUserPosts = async (isInitial = false) => {
  try {
    ...
    const url = limit
      ? `/api/community/posts/?author=${username}&limit=${limit}`
      : `/api/community/posts/?author=${username}`;
    const response = await axios.get(url);
...
const addStock = async (type, stock) => {
  const stockData = { code: stock.stock_code, name: stock.stock_name };
  try {
    const currentStocks =
      type === "owned"
        ? profile.value.owned_stocks
        : profile.value.interested_stocks;
    if (currentStocks.find((s) => s.code === stockData.code)) {
      alert("이미 추가된 주식입니다.");
      return;
    }
    ...
    if (type === "owned") {
      ...
      profile.value.interested_stocks.push(stockData);
...
const removeStock = async (type, stockCode) => {
  try {
    ...
    if (type === "owned") {
      ...
      profile.value.interested_stocks = profile.value.interested_stocks.filter(
        (stock) => stock.code !== stockCode
      );
    }
  } catch (error) {
