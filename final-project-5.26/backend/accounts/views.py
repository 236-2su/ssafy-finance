            ...
            stocks = [
                {
                    "code": "005930",
                    "name": "삼성전자",
                    "reason": "안정적인 대형주로 배당 수익 기대",
                },
                {
                    "code": "000660",
                    "name": "SK하이닉스",
                    "reason": "반도체 업계 선두주자로 안정성 높음",
                },
                {
                    "code": "035420",
                    "name": "NAVER",
                    "reason": "국내 IT 대표주로 꾸준한 성장세",
                },
            ]
            ...
            stocks = [
                {
                    "code": "005930",
                    "name": "삼성전자",
                    "reason": "안정성과 성장성을 겸비한 대형주",
                },
                {
                    "code": "035720",
                    "name": "카카오",
                    "reason": "플랫폼 기업으로 성장 가능성 높음",
                },
                {
                    "code": "207940",
                    "name": "삼성바이오로직스",
            ...
            stocks = [
                {
                    "code": "035720",
                    "name": "카카오",
                    "reason": "디지털 전환 수혜주로 고성장 기대",
                },
                {
                    "code": "373220",
                    "name": "LG에너지솔루션",
                    "reason": "전기차 배터리 시장 선도",
                },
                {
                    "code": "068270",
                    "name": "셀트리온",
                    "reason": "바이오시밀러 글로벌 진출 기대",
                },
            ]
...
class UserStocksView(APIView):
    ...
    def delete(self, request):
        ...
        if stock_type == "owned":
            user.owned_stocks = [
                stock for stock in user.owned_stocks if stock.get("code") != stock_code
            ]
        elif stock_type == "interested":
            user.interested_stocks = [
                stock
                for stock in user.interested_stocks
    ...
    def delete(self, request):
        ...
        if content_type == "video":
            user.watch_later_videos = [
                video
                for video in user.watch_later_videos
                if video.get("video_id") != content_id
            ]
        elif content_type == "channel":
            user.subscribed_channels = [
                channel
                for channel in user.subscribed_channels
                if channel.get("channel_id") != content_id
            ]
...
class UserScrappedPostsView(APIView):
    """사용자가 스크랩한 게시글 목록 API"""

    permission_classes = [IsAuthenticated]

    def get(self, request):
        user = request.user
        # 사용자가 북마크한 게시글 ID 목록 가져오기
        bookmarked_post_ids = Bookmark.objects.filter(user=user).values_list(
            "post_id", flat=True
        )
        # 해당 게시글들을 가져오되, 최신순으로 정렬
        # community.models.Post 를 직접 참조하기보다는, Bookmark 모델을 통해 접근
        # 또는 Post 모델을 직접 import 해서 사용 (여기서는 Bookmark를 통해 가져옴)
        # Post 모델을 직접 사용하려면 from community.models import Post 추가 필요

        # Bookmark 모델을 통해 Post 객체들을 가져옵니다.
        # select_related 또는 prefetch_related를 사용하여 Post 정보를 효율적으로 가져올 수 있습니다.
        bookmarks = (
        ...
        # Post 객체만 추출하여 PostSerializer로 직렬화
        scrapped_posts = [bookmark.post for bookmark in bookmarks]
        serializer = PostSerializer(
            scrapped_posts, many=True, context={"request": request}
        )  # PostSerializer에 request context 전달
        return Response(serializer.data)
